...
.PHONY: help clean check-python pin-pip install install-dev install-test install-hook install-hooks bootstrap-dev lint format typecheck test ci reset-precommit
...
help:
	...
	@echo "  install-dev   Clean and install the package in editable mode with all dev dependencies"
...
check-python:
	@$(PYTHON) -c 'import sys; expected = "$(PYTHON_SERIES)"; actual = f"{sys.version_info.major}.{sys.version_info.minor}"; version = f"{sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro}"; sys.exit(0 if actual == expected else f"Python {expected}.x is required. Current interpreter: {version}.")'
